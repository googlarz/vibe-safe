...
# 26. API key in client-side / frontend file (visible in browser)
FRONTEND_CRED=""
for f in $CHANGED_FILES; do
  case "$f" in
    src/*|frontend/*|public/*|static/*|client/*|components/*|pages/*|app/*)
      FILE_ADDED=$(git diff "origin/$BASE_BRANCH"...HEAD -- "$f" 2>/dev/null | grep "^+" | grep -v "^+++")
...
[ -n "$FRONTEND_CRED" ] && fail "STOP — credential in client-side file: visible to anyone who opens DevTools" "$FRONTEND_CRED" \
  "Move to a server-side environment variable or route the call through a backend proxy."

# 27. Sensitive data in log statements
...
[ -n "$SECRET_LOG" ] && warn "Possible sensitive data in log output — scrub before production" "$SECRET_LOG"

# 28. Security header regression
HELMET_REMOVED=$(echo "$DIFF" | grep "^-" | grep -v "^---" | grep -E "helmet\(\)|require.*['\"]helmet['\"]|import.*['\"]helmet['\"]" 2>/dev/null)
[ -n "$HELMET_REMOVED" ] && warn "helmet removed — verify security headers are still set elsewhere" "$HELMET_REMOVED"
...
[ -n "$INSECURE_HDR" ] && warn "Insecure security header configuration" "$INSECURE_HDR"

# 29. New API route without rate limiting
for f in $CHANGED_FILES; do
  ...
  if [ -n "$NEW_ROUTES" ]; then
    ...
    [ -z "$HAS_RATE" ] && warn "New route in $f has no rate limiting — add before exposing to traffic" "$NEW_ROUTES"
  fi
done
